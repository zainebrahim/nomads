...
from keras.layers import Dense, Conv2D, Conv2DTranspose, Flatten, Input, Reshape, MaxPooling2D, BatchNormalization, Dropout, Concatenate, Softmax, LeakyReLU, Conv3D, Reshape
...
class NomadsBeta:
    ...
    def build_model(self):
        input_tensor = Input((3, 16, 16, self.num_chans))
        conv0 = Conv3D(16,
                       (3, 1, 1),
                       padding='valid',
                       activation='relu')(input_tensor)

        conv1in = Reshape((16, 16, 16))(conv0)

        conv1 = Conv2D(16,
                       (5, 5),
                       padding='same',
                       activation='relu')(conv1in)
